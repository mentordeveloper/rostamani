...
{{-- Create User Form --}}
<form class="form-horizontal" method="post" action="@if (isset($user)){{ URL::to('wtadmin/users/' . $user->id . '/edit') }}@endif" autocomplete="off">
    <!-- CSRF Token -->
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="tab-general">
            ...
            <!-- Grades -->
            <div class="form-group {{{ $errors->has('grades') || $errors->has('grades') ? 'error' : '' }}}">
                <label class="col-md-2 control-label" for="grades">Groups</label>
                <div class="col-md-6">
                    @if ($mode == 'create')
                    <select class="form-control" name="grades" id="grades">
                        <option value="">Please Select Group</option>
                        @foreach ($grades as $grade)
                        <option value="{{{ $grade->id }}}"{{{ ( ($grade->id == $selectedGrade) ? ' selected="selected"' : '') }}}>{{{ $grade->gr_name }}}</option>
                        @endforeach
                    </select>
                    @else
                    <select class="form-control" {{{ ($user->id === Confide::user()->id ? ' disabled="disabled"' : '') }}} name="grades" id="grades">
                        <option value="">Please Select Group</option>
                        @foreach ($grades as $grade)
                        <option value="{{{ $grade->id }}}"{{{ ( ($grade->id==$selectedGrade) ? ' selected="selected"' : '') }}}>{{{ $grade->gr_name }}}</option>
                        @endforeach
                    ...
                    @endif
                    {{ $errors->first('grades', '<span class="help-inline">:message</span>') }}
                </div>
            </div>
            <!-- ./ activation status -->
