...
function initCarousel() {
    ...
    jQuery('.pp-carousel').scrollGallery({
        mask: '.mask',
        slider: '.slideset',
        slides: '.slide',
        btnPrev: 'a.btn-prev',
    ...
    jQuery('.tab-carousel').scrollGallery({
        mask: '.mask',
        slider: '.slideset',
        slides: '.slide',
        btnPrev: 'a.btn-prev',
        ...
        stretchSlideToMask: true,
        maskAutoSize: true,
//		generatePagination: '.pagination_new',
        autoRotation: false,
        switchTime: 3000,
        animSpeed: 500
    });
...
function initTabs() {
    ...
    jQuery('.main-p-holder').contentTabs({
        autoHeight: true,
        checkHash: true,
        animSpeed: 400,
        effect: 'slide',
...
function initMobileNav() {
    ...
    jQuery('#header').mobileNav({
        hideOnClickOutside: true,
        menuActiveClass: 'nav-active1',
        menuOpener: '.nav-opener1',
        menuDrop: '#nav ul'
    });
    jQuery('.lamp-holder').mobileNav({
        hideOnClickOutside: true,
        menuActiveClass: 'active',
        menuOpener: '.lamp',
        menuDrop: '.lamp-holder ul'
    });
}
    ...
    $.fn.contentTabs = function (o) {
        ...
        return this.each(function () {
            ...
            function switchTab(oldLink, newLink) {
                $(".btn-close-sub").parent().addClass(tabHiddenClass);
                var activeItemm = (options.addToParent ? tabLinksParents : tabLinks).filter('.' + options.activeClass);
                var activeIndexx = (options.addToParent ? tabLinksParents : tabLinks).index(activeItemm);
                var newLinkk = tabLinks.eq(activeIndexx < tabLinks.length - 1 ? activeIndexx + 1 : 0);
                prevActiveLinkk = tabLinks.eq(activeIndexx);
                resizeHolder(newLinkk, false);
                    ...
                    complete: function () {
                        ...
                        contentTabsEffect[options.effect].show({
                            ...
                            complete: function () {
                                ...
                                $('html,body').animate({
                                    scrollTop: newTab.offset().top - 70
                                }, 100);
                            }
                        });
                    }
            ...
            // close single tab
            function close_nextTab() {
                var activeItem = (options.addToParent ? tabLinksParents : tabLinks).filter('.' + options.activeClass);
                var activeIndex = (options.addToParent ? tabLinksParents : tabLinks).index(activeItem);
                var newLink = tabLinks.eq(activeIndex < tabLinks.length - 1 ? activeIndex + 1 : 0);
                prevActiveLink = tabLinks.eq(activeIndex);
                close_switchTab(prevActiveLink, newLink);
            }

            function close_switchTab(oldLink, newLink) {
                animating = true;
                var oldTab = oldLink.data('ctab');
                var newTab = newLink.data('ctab');
                prevActiveLink = newLink;
            ...
            $(".btn-close-sub").click(function () {
                $(this).parent().addClass(tabHiddenClass);
                var activeItem = (options.addToParent ? tabLinksParents : tabLinks).filter('.' + options.activeClass);
                var activeIndex = (options.addToParent ? tabLinksParents : tabLinks).index(activeItem);
                var newLink = tabLinks.eq(activeIndex < tabLinks.length - 1 ? activeIndex + 1 : 0);
                prevActiveLink = tabLinks.eq(activeIndex);
                $('html,body').animate({
                    scrollTop: $('#main_tab_all').offset().top
                }, 100);

                resizeHolder(newLink, false);
                // autoRotate();
                // $(window).trigger('resize');
                // close_nextTab();
...
jQuery(document).on("click", ".indoor-tab a", function () {
    console.log("Slide Active: " + jQuery(this).data('slide'));
    jQuery(jQuery(this).data('slide')).addClass('active');
    jQuery('#sub_tab_content').slideDown('fast');
});
jQuery(document).on("click", ".electrical_no_bg", function () {
    var link = jQuery(this).data('href');
    console.log("Href: " + link);
    window.open(link, '_blank');
    jQuery('#sub_tab_content').hide();
});
    ...
    jQuery('.tp-banner').show().revolution(
            {
                ...
                startWithSlide: 0,
                videoJsPath: "/assets/rev_slider/rs-plugin/videojs/",
